...
def file_input2(request):
    ...
    if request.POST and request.FILES:
        ...
        voca = nltk.Text(nouns)
        data = voca.vocab().most_common(500)
        tmp_data = dict(data)
        font_name = font_manager.FontProperties(fname="/usr/share/fonts/truetype/nanum/NanumGothic.ttf").get_name()
        rc('font', family=font_name)
        wc = wordcloud.WordCloud(font_path="/usr/share/fonts/truetype/nanum/NanumGothic.ttf",relative_scaling=0.2, background_color="white").generate_from_frequencies(tmp_data)
        ...
        return render_to_response('blog_test/file_input2.html', {'figure_wc': fig_html})
    #list = ['평창', '동계', '올림픽']
    return render(request, 'blog_test/file_input2.html', locals())
